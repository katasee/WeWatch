...
internal final class HomeViewModel: ObservableObject {
    ...
    // cторінка
    var dbManager: DatabaseManager = .init(dataBaseName: database.name)
    ...
    internal func dataForTodaySelection() async throws {
        if hasDateChanged() {
            ...
            await MainActor.run { [weak self] in
                Task {
                    self?.todaySelection = try await dbManager.fetchMovieByList(forList: Constans.todaySelectionList)
